        ...
        <div class="col-md-8">
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    ...
                    {{-- Отдельная кнопка для смены пароля --}}
                    <form method="POST" action="{{ route('profile.sendResetLink') }}">
                        @csrf
                        <button type="submit" class="btn btn-warning">Сменить пароль</button>
                        <small class="form-text text-muted">На ваш email будет отправлена ссылка для смены пароля.</small>
                    </form>
                </div>
            </div>
        </div>
